...
The control-plane image also bundles [Anthropic's official skills](https://github.com/anthropics/skills) (docx, pptx, xlsx, pdf, mcp-builder, skill-creator, …), fetched at build time pinned to a commit (`ANTHROPIC_SKILLS_REF` in the Dockerfile), so they seed the same way. The skill store holds text files up to 256KB — binary or oversized files are skipped at seed with a logged warning, and `canvas-design` / `web-artifacts-builder` are excluded outright because their core assets (fonts, a tar.gz) are binary. Note some of these skills expect tooling in the sandbox image (Python libraries, LibreOffice, …) that may not be installed; treat their instructions as best-effort until the sandbox image carries the dependencies they name.
