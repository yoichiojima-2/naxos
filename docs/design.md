...
Internal surface (`naxos-internal`, IAM-only): per-session `claim / heartbeat / queue?wait / events / stream (transient partial-text fan-out) / checkpoint / config / skills / memory_writeback / artifacts (list·register·delete·share) / deployments (list·create·archive)`, plus `deployments/{id}/fire` and `reconcile`.
