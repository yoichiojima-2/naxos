...
- **Skills** — org-shared Agent Skills folders (`SKILL.md` + files) stored in Postgres and attached via `agent_versions.skill_ids`; mounted read-only into the sandbox at `ws/.claude/skills/{name}/` each wake. No writeback path — skills are edited only through the API. `Skill` invocations go through the normal `PreToolUse` permission gate and audit.
