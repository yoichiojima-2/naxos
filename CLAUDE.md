...
- **Events** — CMA vocabulary (`user.message`, `user.interrupt`, `user.tool_confirmation`, `agent.message`, `agent.tool_use`, `agent.artifact`, `session.status_idle` with stop_reason, `span.model_request_*`, …), append-only per-session `seq`, SSE streaming with `Last-Event-ID` replay.
...
- **Audit**: the execution record is the Postgres `tool_calls` table, written **at the permission gate** — the one point every tool call must pass — so the row is committed before the tool runs and does not depend on the sandbox surviving to report it. One row per call: who (the principal of the turn, and the approver on a gated call), which tool, the canonical args the `call_hash` covers, the decision (`auto_allowed|user_allowed|user_denied|not_allowed|killed|awaiting_confirmation`), and the outcome (`ok|error|denied|no_result`, latency, error) joined from the tool result. It has no FK to `sessions`, so deleting a session cannot erase it. BigQuery `naxos_audit.tool_calls` is an append-only export of completed rows written at each checkpoint (a streamed row cannot be updated, so it is never written before the call finishes); `runs` gets one row per wake-to-idle burst with principal, tokens and cost. BigQuery stays write-only from the control plane — `GET /v1/tool_calls` and the monitoring view read Postgres, so the API never needs BigQuery read access.
...
- **Approval gate**: permission policy `always_ask` → pending `tool_confirmations` row → `session.status_idle(requires_action)` → container released while waiting → `user.tool_confirmation` resumes. Waiting is not the same as being seen, so the gate also has a **queue, a signal and a deadline**: `GET /v1/tool_confirmations` is a cross-session inbox (the UI's Approvals view, with a pending-count badge on every page), the gate logs `naxos.approval_required` for a Terraform-provisioned log-based alert that mails `alert_email`, and the reconciler expires anything older than `CONFIRMATION_TTL_HOURS` — resuming the session down the same path a human decision takes, so a forgotten approval unblocks the agent instead of parking it forever. Deciding is always `POST /v1/sessions/{id}/events`; the inbox carries the `session_id` and `call_hash` that call needs rather than adding a second decision path.
