...
- **Audit**: every tool call → `audit.tool_calls` with decision (`auto_allowed|user_allowed|user_denied|not_allowed|killed`); every wake-to-idle burst → `audit.runs` with principal and cost.
