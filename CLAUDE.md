...
**Status (2026-07-28): the Phase 1 loop is built and running** — daily Scheduler → per-role Cloud Run Jobs → Agent SDK (Anthropic API key exception, see below) → Slack, with `audit.runs`, kill switch, and CI/CD (GitHub Actions + WIF). Phase-1 stand-ins that upgrade later, with the seams already in place: "tenants" are `roles.json` roles (no Cloud SQL yet — the tenant concept is deliberately parked); the kill switch is a GCS marker (`gs://$BUCKET/disabled/<role>`) checked at run start only — the before-every-tool-call check specified under Governance is not yet implemented; audit is a single `runs` table with tool calls embedded as JSON (no separate `tool_calls` table, no `principal` until IAP exists); sessions persist to `gs://$BUCKET/sessions/` for cross-run `--resume`.

An early slice of Phase 2 is also deployed and working: `naxos-ui` (Next.js + FastAPI on the same image) fronted by **IAP directly on Cloud Run rather than LB → IAP** — the LB is only needed for a custom domain, and skipping it saves the ~¥3k/month it would cost. Because the project has no parent organization, IAP runs on a custom OAuth client (consent screen in Testing mode; onboarding = OAuth test user + IAP IAM binding). The UI covers chat (NDJSON status streaming, markdown replies, IAP principal into `audit.runs.principal`), run history with session reopen, and a schedules tab that lists and manages scheduled tasks (create/edit/pause/delete, any number per role) as Cloud Scheduler jobs — tasks are user data owned by the UI/gcloud; Terraform owns only the scheduler SA, its invoker grant, and the UI's custom editor role. A run-now button force-runs a task through the same runner path as a cron firing, so kill switch and audit apply unchanged. Multi-turn chat holds a live SDK client per session in instance memory (Cloud Run session affinity), falling back to `--resume` when the instance is gone. Schedule creation from chat follows propose-then-confirm: the `propose_schedule` tool writes nothing, the UI prefills a form from the tool-call event, and a signed-in human saves — a deliberate approval-gate-lite until the real gate ships. The approval gate and DLP remain Phase 2 work.
...
- Infrastructure as code (Terraform) owns the topology: service accounts, IAM bindings, BigQuery, Cloud SQL, Secret Manager secrets (containers, not values), budget alerts, DLP templates. Keep it boring: one root module, GCS state backend, no module abstraction.
- gcloud/CI/UI owns the day-to-day: image deploys (`gcloud run deploy` with `lifecycle { ignore_changes = [image] }` on the Terraform side), secret *values* (`gcloud secrets versions add` — never in state or git), scheduled tasks (`naxos-schedule-*` Cloud Scheduler jobs — user data created/edited/deleted via the UI or gcloud, no apply involved; Terraform owns only the surrounding IAM), ad-hoc ops.
